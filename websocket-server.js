  ...
  if (
    ...
    keepAliveInterval = setInterval(async () => {
      try {
        ...
        http
          .get(`${serviceUrl}/health`, (res) => {
            const duration = Date.now() - startTime;
            console.log(
              `✅ Keep-alive ping successful (${duration}ms) - Status: ${res.statusCode}`
            );
          })
          .on("error", (err) => {
...
function getAvatarUrl(user, size = 64) {
  if (!user) {
    return null; // ✅ Return null instead of generating
  }
  ...
  const primaryAvatar =
    user.avatars?.find((a) => a.isPrimary) || user.avatars?.[0];
  ...
  if (primaryAvatar) {
    ...
    // Priority 2b: Generated avatar with custom colors (user explicitly chose this)
    if (primaryAvatar.avatarIndex >= 0) {
  ...
  // ✅ REMOVED: Fallback Dicebear generation
  // Return null so frontend shows default red icon
  return null;
}
    ...
    try {
      ...
      console.log("🔐 [AUTH] Token payload (no verification):", {
        ...
        exp: decodedNoVerify?.exp
          ? new Date(decodedNoVerify.exp * 1000).toISOString()
          : "No expiry",
        iat: decodedNoVerify?.iat
          ? new Date(decodedNoVerify.iat * 1000).toISOString()
          : "No issued at",
      });
          ...
          try {
            ...
            const chatRoom = await prisma.chatRoom.findUnique({
              ...
              include: {
                course: {
                  select: {
                    userId: true  // ✅ CORRECT: Course creator field
                  }
                }
              }
            });
            ...
            // ✅ DETERMINE ROLE BEFORE CHECKING PARTICIPANT
            const isCourseOwner = chatRoom?.course?.userId === user.id;
            ...
            // ✅ CREATE PARTICIPANT IF DOESN'T EXIST (for course owner)
            if (!participant && isCourseOwner) {
            ...
            // ✅ UPDATE PARTICIPANT WITH CORRECT ROLE
            ...
            console.log(`✅ WS: User ${user.username} joined room as ${userRole}${isCourseOwner ? ' (COURSE OWNER)' : ''}`);

            // Send confirmation
            ws.send(
              JSON.stringify({
                event: "room_joined",
                data: { roomId, userId: user.id, role: userRole },
              })
            );

            // ✅ BROADCAST WITH CORRECT ROLE
           broadcastToRoom(
  roomId,
  "user:online",
  {
    ...
    role: userRole, // ✅ Will be 'mentor' ONLY for course owner
  ...
  try {
    ...
    if (!hasValidContent && !hasMedia) {
      ws.send(JSON.stringify({
        event: 'error',
        data: { 
          message: 'Either text content or media is required', 
          code: 'EMPTY_MESSAGE' 
        }
      }));
      return;
    ...
    if (hasValidContent) {
      ...
      const encrypted = encryptMessage(mediaFileName || '[Media]');
      encryptedContent = encrypted.encryptedContent;
      contentHash = encrypted.contentHash;
      actualContent = '';
    }

    const wordCount = hasValidContent ? content.trim().split(/\s+/).length : 0;
    const characterCount = hasValidContent ? content.length : 0;

    // ✅ GET COURSE OWNER TO DETERMINE ROLE
    const chatRoom = await prisma.chatRoom.findUnique({
      where: { id: roomId },
      include: {
        course: {
    ...
    console.log(`🔍 Role Check - User: ${user.username}, Is Owner: ${isCourseOwner}, Course Owner: ${chatRoom?.course?.userId}`);
    ...
    const message = await prisma.chatMessage.create({
      data: {
        ...
        mediaFileSize: mediaFileSize ? parseInt(mediaFileSize.toString()) : null,
        mediaPublicId: mediaPublicId || null,
        mediaWidth: mediaWidth ? parseInt(mediaWidth.toString()) : null,
        mediaHeight: mediaHeight ? parseInt(mediaHeight.toString()) : null,
        mediaDuration: mediaDuration ? parseFloat(mediaDuration.toString()) : null,
    ...
    if (isCourseOwner) {
      userType = 'tutor';
      userRole = 'mentor';
      console.log(`✅ User ${user.username} is COURSE OWNER - marked as MENTOR`);
    } else {
      userRole = 'student';
      userType = 'learner';
      console.log(`📝 User ${user.username} is STUDENT`);
    }
    ...
    // ✅ COMPLETE MESSAGE DATA WITH ROLE IN USER OBJECT
    const messageData = {
      id: message.id,
      ...
      // ✅ CRITICAL: Add role to user object
      user: {
        ...message.user,
        avatar: avatarUrl,
        avatarObject: primaryAvatar,
        role: userRole, // ✅ THIS IS THE KEY FIX
        type: userType,
      },
      ...
      replyTo: message.replyTo ? {
        ...
        user: {
          ...
          avatar: getAvatarUrl(message.replyTo.user, 48),
    ...
    console.log('✅ Broadcasting message:', {
      ...
      role: messageData.user.role, // ✅ Log the role from user object
      ...
      try {
        ...
        if (roomId) {
          ...
          if (roomConnections) {
            ...
            console.log(
              `✅ WS: Broadcasted offline status for ${user.username}`
            );
          }
        }
      } catch (error) {
